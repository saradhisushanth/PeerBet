...
export async function rebalanceMatchIfLocked(matchId: string): Promise<void> {
  ...
  const participantIds = [...new Set(pendingBets.map((b) => b.userId))];
  const leaderboardEntries = await prisma.leaderboard.findMany({
    where: { userId: { in: participantIds } },
    select: { userId: true, rank: true },
  });
  const rankByUser = new Map(leaderboardEntries.map((e) => [e.userId, e.rank ?? 999999]));

  const participantsWithRank = pendingBets.map((b) => ({
    betId: b.id,
    userId: b.userId,
  ...
  const uniqueParticipants = [...byUserId.values()].sort(
    (a, b) => b.rank - a.rank || a.userId.localeCompare(b.userId)
  );

  const homeStake = pendingBets.filter((b) => b.selectedTeamId === match.homeTeamId).reduce((s, b) => s + b.amount, 0);
  const awayStake = pendingBets.filter((b) => b.selectedTeamId === match.awayTeamId).reduce((s, b) => s + b.amount, 0);
  const homeCount = pendingBets.filter((b) => b.selectedTeamId === match.homeTeamId).length;
  const awayCount = pendingBets.filter((b) => b.selectedTeamId === match.awayTeamId).length;
  ...
  const movesToDo = toMove.filter((p) => p.selectedTeamId !== otherSideTeamId);
