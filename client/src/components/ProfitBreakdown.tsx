...
function DonutChart({ slices, size = 140 }: { slices: Slice[]; size?: number }) {
  ...
  const paths = slices
    ...
    .map((slice) => {
      ...
      return <path key={slice.label} d={d} fill={slice.color} stroke="#111827" strokeWidth="1.5" />;
    });

  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="shrink-0">
      {paths}
      <text x={cx} y={cy - 6} textAnchor="middle" className="fill-gray-400 text-[9px]">
        Net Profit
      </text>
      <text x={cx} y={cy + 10} textAnchor="middle" className="fill-white text-sm font-bold">
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
      ...
      <div className="px-3 py-4 flex items-start gap-4">
        ...
        <div className="flex-1 space-y-2 min-w-0">
          ...
          {slices.length === 0 && (
            <p className="text-gray-500 text-xs">No profit components to display.</p>
          )}
        </div>
      </div>

      <div className="border-t border-gray-800 px-3 py-3 space-y-3">
        <h4 className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">How it's calculated</h4>
        ...
        <div className="space-y-1">
          ...
          <div className="bg-gray-800/60 rounded-lg px-2.5 py-2 font-mono text-[11px] text-gray-300 leading-relaxed">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-1 text-white">
              <span className="text-gray-500">(</span>
        ...
        <div className="space-y-1">
          ...
          {isUnderdog ? (
            ...
            <p className="text-gray-600 text-[11px] px-1">
              You backed <span className="text-gray-400">{playerSide}</span> (favourite) — no underdog bonus.
            </p>
          )}
        </div>
        ...
        <div className="space-y-1">
          ...
          {streakBonus > 0 ? (
            <div className="bg-gray-800/60 rounded-lg px-2.5 py-2 font-mono text-[11px] text-gray-300 leading-relaxed">
              <div className="flex flex-wrap items-center gap-x-1 text-white">
            ...
            <p className="text-gray-600 text-[11px] px-1">No streak bonus this match.</p>
          )}
        </div>
